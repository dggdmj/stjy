...
class WagesJxbAction extends WagesCommonAction{
    ...
    public function index(){
        ...
        if ($suoshudd){
            ...
            $list_pz = M('jxbgz_pz')->where("suoshudd='$suoshudd'")->order('id asc')->select();
    ...
    public function getInfo(){
        $shenfenzhm = I('shenfenzhm','440181198408305441');
        $info = M('rycb')->field('bumen,xingming,shenfenzhm,zhiwu as zhiwei,gangweilx,leixing as zaizhizt,xingming,ruzhirq as ruzhisj,erjibm,diyixlbysj')->where(array('shenfenzhm'=>$shenfenzhm))->find();
        dump($info);
    }

    //根据姓名获取身份证
    public function getSfz(){
        $xingming = I('xingming');
        $list = M('rycb')->field('shenfenzhm')->where(array('xingming'=>$xingming))->select();
    ...
    public function saves(){
        ...
        M('jxbgz_pz')->where("suoshudd='$suoshudd'")->delete();
        $list = array();
...
=IF(
    BE6>=0,
    IF( 
        OR(封面!$D$3="A类",封面!$D$3="A+类",封面!$D$3="A-类"),
        IF(BB6>=600000,
            IF(BD6<=300,BE6*30,
                IF(BC6>=300,BE6*200,(BD6-300)*200+(300-BC6)*30)
            ),
            IF(BD6<=300,BE6*30*0.5,
                IF(BC6>=300,BE6*200*0.5,(BD6-300)*200*0.5+(300-BC6)*30*0.5)
            )
        ),
        IF(封面!$D$3="B类",
            IF(BB6>=600000,
            ...
            IF(封面!$D$3="D类",
                IF(BB6>=600000,
                    IF(BD6<=300,BE6*8,
                    IF(BC6>=300,BE6*54,(BD6-300)*54+(300-BC6)*8)),
                    IF(BD6<=300,BE6*8*0.5,
                    IF(BC6>=300,BE6*54*0.5,(BD6-300)*54*0.5+(300-BC6)*8*0.5))
            ),

            IF(封面!$D$3="C类",
                IF(BB6>=600000,
                    IF(BD6<=500,BE6*18,
                        IF(BC6>=500,BE6*120,(BD6-500)*120+(500-BC6)*18)
                    ),
                    IF(BD6<=500,BE6*18*0.5,
                        IF(BC6>=500,BE6*120*0.5,(BD6-500)*120*0.5+(500-BC6)*18*0.5)
                    )
                ),0)
            )
        )
    ...
    IF(
        OR(封面!$D$3="A类",封面!$D$3="A+类",封面!$D$3="A-类"),
            IF(BC6<=300,BE6*30,
            IF(BD6>=300,BE6*200,(BC6-300)*200+(300-BD6)*30)),
        IF(封面!$D$3="B类",
            IF(BC6<=300,BE6*30,
            IF(BD6>=300,BE6*200,(BC6-300)*200+(300-BD6)*30))*0.8,
        IF(封面!$D$3="D类",
            IF(BC6<=300,BE6*8,
            IF(BD6>=300,BE6*54,(BC6-300)*54+(300-BD6)*8)),
        IF(封面!$D$3="C类",
            IF(BC6<=500,BE6*18,
            IF(BD6>=500,BE6*120,(BC6-500)*120+(500-BD6)*18)),
        0)
        )
    )
    )
